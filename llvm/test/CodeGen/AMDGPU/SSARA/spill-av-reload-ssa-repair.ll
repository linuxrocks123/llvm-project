; RUN: llc -mtriple=amdgcn-amd-amdhsa -mcpu=gfx908 -amdgpu-ssa-regalloc -verify-machineinstrs < %s | FileCheck %s

; Guard for recognizing AGPR-or-VGPR (AV) reloads during SSA repair
; (AMDGPUSSARegisterSpiller isSpillInstr/isReloadInstr, classified by TSFlag).
;
; On gfx90a+ the spiller may reload through an AV class as SI_SPILL_AV32_RESTORE.
; The old hand-maintained opcode list matched only SGPR/VGPR spill pseudos, so an
; AV reload was not recognized, its redef was never renamed during SSA repair,
; and the spilled vreg ended up multiply-defined -> getVRegDef assertion. The fix
; classifies spill/reload by the Spill TSFlag (covers SGPR/VGPR/AGPR/AV).
